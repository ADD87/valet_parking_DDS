...
bool ValetParkingComponent::InitDds() {
  ...
  raw_topic_type_ = std::make_unique<magna::dds::TopicDataType_raw>(kRawTypeName);
  magna::dds::ReturnCode_t rc = participant_->register_type(raw_topic_type_.get());
  if (rc != magna::dds::ReturnCode_t::RETCODE_OK) {
    SetLastError(std::string("register_type failed: ") + ReturnCodeToString(rc));
    CleanupDds();
    return false;
  }

  input_topic_ = participant_->create_topic(input_topic_name_, raw_topic_type_->get_name(),
  ...
  output_topic_ = participant_->create_topic(output_topic_name_, raw_topic_type_->get_name(),
                                             magna::dds::TOPIC_QOS_DEFAULT);
...
void ValetParkingComponent::CleanupDds() noexcept {
  ...
  raw_topic_type_.reset();
}

bool ValetParkingComponent::BuildTrajectoryPayloadFromInput(const std::string& input_payload,
                                                            std::string* output_payload,
                                                            std::string* status_reason) const {
  if (output_payload == nullptr || status_reason == nullptr) {
    return false;
  }

  const SelectedSlotInput input = DecodeSelectedSlotInput(input_payload);
  *status_reason = input.status_reason;
  *output_payload = BuildTrajectoryJson(config_, input);
  return true;
}
...
bool ValetParkingComponent::HandleOneSample() {
  ...
  magna::dds::OctetSeq raw_input;
  magna::dds::SampleInfo sample_info;
  const magna::dds::ReturnCode_t read_rc =
      input_reader_->take_next_sample_original(raw_input, sample_info);
  ...
  if (read_rc != magna::dds::ReturnCode_t::RETCODE_OK) {
    SetLastError(std::string("take_next_sample_original failed: ") + ReturnCodeToString(read_rc));
    return false;
  }
  ...
  const std::string input_payload(raw_input.begin(), raw_input.end());
  std::string output_payload;
  std::string status_reason;
  if (!BuildTrajectoryPayloadFromInput(input_payload, &output_payload, &status_reason)) {
    SetLastError("failed to build output payload");
    return true;
  }

  const magna::dds::OctetSeq raw_output(output_payload.begin(), output_payload.end());
  const magna::dds::ReturnCode_t write_rc = output_writer_->write_original(raw_output);
  if (write_rc != magna::dds::ReturnCode_t::RETCODE_OK) {
    SetLastError(std::string("write_original failed: ") + ReturnCodeToString(write_rc));
    return true;
  }
  ...
  std::cout << kModuleTag
            ...
            << " (input_bytes=" << raw_input.size()
            << ", output_bytes=" << raw_output.size()
