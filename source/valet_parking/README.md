...
这仍只是完整 `OpenSpacePathProvider` 的轻量切片，不包含完整线程管理、NLP smoother、完整 `PreCheck` 或完整 `Frame/DependencyInjector`。当前 `moving-localization` mock 已能直接触发并验证 `warm_start=history_splice` 分支；`far-localization` mock 已能验证错 frame/远定位会被前置 estop；但真实车端定位/底盘/障碍物 Topic 仍未对齐。

## 最近验证

NEXT-021 轻量 PreCheck/输入边界切片后，已验证：
