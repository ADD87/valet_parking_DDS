...
bool SCSShapePath::GenerateSCSShapePath(
  ...
  Trans2LocalCoor(start_node, &end_node_local, &dest_region_with_angle_local,
                  &quadrant);
  double radius = std::fabs(start_node.kappa) > kEpsilon
...
void SCSShapePath::Trans2LocalCoor(
  ...
  *quadrant = 1;
  ...
  end_node_ptr->x = relative_point.x();
  end_node_ptr->y = relative_point.y();
  end_node_ptr->theta = relative_angle;
  const auto& origin_polygon2d = std::get<0>(*dest_region_with_angle_ptr);
  if (origin_polygon2d.num_points() > 2) {
    auto points = origin_polygon2d.points();
...
bool SCSShapePath::GetSLSInfo(const DestRegionWithAng& dest_region_with_angle,
  ...
  int edge_num = is_polygon_valid ? static_cast<int>(polygon2d.line_segments().size()) : 4;
  ...
  if (is_polygon_valid) {
    TransPolygon2HyperPlan(polygon2d, A.block(0, 0, edge_num, 3), b.head(edge_num));
  } else {
    A(0, 0) = 1;
  ...
  target_pose_ptr->x = x[0];
  target_pose_ptr->y = x[1];
  *radius_ptr = x[2];
  return true;
}
