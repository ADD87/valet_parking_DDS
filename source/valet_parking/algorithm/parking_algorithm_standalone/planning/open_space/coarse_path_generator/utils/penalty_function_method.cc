...
void TransPolygon2HyperPlan(const common::math::Polygon2d& polygon,
  ...
  for (int i = 0; i < pt_size; ++i) {
    ...
    if (fabs(points[i].x() - points[ni].x()) < kAccuracy) {
      ...
      double k = (points[ni].y() - points[i].y()) /
...
bool PenaltyFunctionMethod::OptimizeWithOSQP(
    Eigen::Ref<Eigen::Vector3d> x) {
  // NOLINTBEGIN
  ...
  if (work == nullptr) {
    AERROR << "planning failed: osqp_work is nullptr! exitflag is "
           << exitflag;
    c_free(data->A);
    c_free(data->P);
    c_free(data);
