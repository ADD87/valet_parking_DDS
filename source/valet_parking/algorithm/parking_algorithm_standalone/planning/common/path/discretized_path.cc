...
#include "proto/planning/sl_boundary.pb.h"  // 独立编译改造：.cc文件需要完整类型定义
...
double DiscretizedPath::TotalLength() const {
...
PathPoint DiscretizedPath::EvaluateForGreaterThanMaxS(
    const double path_s) const {
  ACHECK(!empty());
  auto it_lower = QueryLowerBound(path_s);
  if (it_lower != begin() && it_lower != end()) {
    return common::math::InterpolateUsingLinearApproximation(*(it_lower - 1),
                                                             *it_lower, path_s);
  }
  if (it_lower == end()) {
  ...
  const auto extend_s = path_s - it_lower->s;
  const auto x_val = it_lower->x + std::cos(it_lower->theta) * extend_s;
  const auto y_val = it_lower->y + std::sin(it_lower->theta) * extend_s;
  PathPoint path_point;
  path_point.x = x_val;
  path_point.y = y_val;
  path_point.z = 0.0;
  path_point.theta = it_lower->theta;
  path_point.kappa = it_lower->kappa;
  path_point.dkappa = it_lower->dkappa;
  path_point.ddkappa = it_lower->ddkappa;
  path_point.s = path_s;
  return path_point;
}
...
bool DiscretizedPath::GetSLBoundary(const common::math::Box2d& box,
                                    SLBoundary* const sl_boundary) const {
  if (sl_boundary == nullptr || empty()) {
    return false;
  }
  ...
  for (const auto& point : corners) {
    auto sl_point = common::math::PathMatcher::GetPathFrenetCoordinate(
...
void DiscretizedPath::GenerateStopPath(const double x, const double y,
                                       const double theta, const double kappa) {
  clear();
  reserve(FLAGS_publish_trajectory_points_number);
  PathPoint path_point;
  path_point.x = x;
  path_point.y = y;
