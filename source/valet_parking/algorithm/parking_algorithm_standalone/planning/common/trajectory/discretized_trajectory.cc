 ...
 * 独立编译改造：
 *   - RepeatedPtrField<TrajectoryPoint> → std::vector<TrajectoryPoint>
 *   - Proto accessor (e.g. .path_point().x()) → struct member access (e.g. .path_point.x)
 *   - ADCTrajectory 构造函数移除
 *   - soc::Chassis::GEAR_xxx → soc::GearPosition::GEAR_xxx
 **/
...
size_t DiscretizedTrajectory::QueryLowerBoundPoint(const double relative_time,
  ...
  if (relative_time >= trajectory_.back().relative_time) {
...
void DiscretizedTrajectory::AppendTrajectoryPoint(
  ...
  if (!trajectory_.empty()) {
    CHECK_GT(trajectory_point.relative_time,
             trajectory_.back().relative_time);
  }
...
TrajectoryPoint* DiscretizedTrajectory::AppendTrajectoryPoint(
    const double relative_time) {
  if (!trajectory_.empty()) {
    CHECK_GT(relative_time, trajectory_.back().relative_time);
  }
...
double DiscretizedTrajectory::GetTemporalLength() const {
  ...
  return trajectory_.front().relative_time -
         trajectory_.back().relative_time;
}
...
double DiscretizedTrajectory::GetSpatialLength() const {
  ...
  return trajectory_.back().path_point.s -
         trajectory_.front().path_point.s;
}

size_t DiscretizedTrajectory::GetTrajectorySegmentSize(
    const TL::soc::GearPosition& init_gear) const {
  ...
  auto gear = [](const common::TrajectoryPoint& from_point,
    ...
    if (diff_angle < M_PI_2) {
      return soc::GearPosition::GEAR_DRIVE;
    }
    return soc::GearPosition::GEAR_REVERSE;
  };
  if (NumOfPoints() > 2) {
    ...
    for (size_t i = 1; i < NumOfPoints(); ++i) {
      ...
      if (current_gear_status != gear_position) {
        if (current_gear_status == soc::GearPosition::GEAR_DRIVE ||
            current_gear_status == soc::GearPosition::GEAR_REVERSE) {
