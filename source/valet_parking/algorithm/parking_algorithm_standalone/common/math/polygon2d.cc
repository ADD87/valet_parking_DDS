...
bool Polygon2d::IsPointIn(const Vec2d& point) const {
  ...
  for (int i = 0; i < num_points_; ++i) {
    if ((points_[i].y() > point.y()) != (points_[j].y() > point.y())) {
      const double side = CrossProd(points_[i].x() - point.x(), points_[i].y() - point.y(),
                                    points_[j].x() - point.x(), points_[j].y() - point.y());
  ...
  static constexpr u_int32_t kBase = 0x01;
  return (c & kBase) > 0;
}
...
void Polygon2d::BuildFromPoints() {
  ...
  for (int i = 1; i < num_points_; ++i) {
    area_ += CrossProd(points_[i-1].x() - points_[0].x(), points_[i-1].y() - points_[0].y(),
                       points_[i].x() - points_[0].x(), points_[i].y() - points_[0].y());
  }
  if (area_ < 0) {
    area_ = -area_;
  ...
  is_convex_ = true;
  for (int i = 0; i < num_points_; ++i) {
    Vec2d v1 = points_[i] - points_[Prev(i)];
    Vec2d v2 = points_[Next(i)] - points_[i];
    if (CrossProd(v1.x(), v1.y(), v2.x(), v2.y()) <= -kMathEpsilon) {
      is_convex_ = false;
      break;
    }
  }
...
bool Polygon2d::ComputeConvexHull(const std::vector<Vec2d>& points,
  ...
  for (int i = 0; i < n + n; ++i) {
    ...
    while (count > last_count) {
      Vec2d v1 = points[results[count - 1]] - points[results[count - 2]];
      Vec2d v2 = pt - points[results[count - 1]];
      if (CrossProd(v1.x(), v1.y(), v2.x(), v2.y()) <= kMathEpsilon) {
        results.pop_back();
        --count;
      } else {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    if (i > 0) {
      area += CrossProd(result_points[i-1].x() - result_points[0].x(),
                        result_points[i-1].y() - result_points[0].y(),
                        result_points[i].x() - result_points[0].x(),
                        result_points[i].y() - result_points[0].y());
    }
  }

  if (fabs(area / 2.0) <= kMathEpsilon) {
    return false;
  }
...
bool Polygon2d::IsConvexPolygon(
  ...
  common::math::Vec2d vec_1(points[0] - points[psize - 1]);
  common::math::Vec2d vec_2(points[1] - points[0]);

  const double signal = vec_1.CrossProd(vec_2);

  for (size_t i = 1; i < psize; ++i) {
    vec_1 = points[i] - points[i - 1];
    vec_2 = points[(i + 1) % psize] - points[i];
    if (signal * vec_1.CrossProd(vec_2) <= kMathEpsilon) {
      return false;
    }
  }
  return true;
...
bool Polygon2d::ClipConvexHull(const LineSegment2d& line_segment,
  ...
  for (size_t i = 0; i < n; ++i) {
    // CrossProd(start, end, point) = (end-start) x (point-start)
    const auto& s = line_segment.start();
    const auto& e = line_segment.end();
    const auto& p = (*points)[i];
    prod[i] = CrossProd(e.x() - s.x(), e.y() - s.y(),
                        p.x() - s.x(), p.y() - s.y());
...
double Polygon2d::ComputeIoU(const Polygon2d& other_polygon) const {
  ...
  if (union_area <= kMathEpsilon) return 0.0;
...
bool Polygon2d::HasOverlap(const LineSegment2d& line_segment) const {
  ...
  // Check if either end point is inside the polygon
  if (IsPointIn(line_segment.start()) || IsPointIn(line_segment.end())) {
    return true;
  }

  // Check if the line segment intersects with any edge of the polygon
  for (const auto& poly_seg : line_segments_) {
    if (poly_seg.HasIntersect(line_segment)) {
      return true;
    }
  }
...
/*
// Commenting out - not declared in header
...
AABox2d Polygon2d::AABoundingBox() const {
  AABox2d aa_box_2d({min_x_, min_y_}, {max_x_, max_y_});
  return aa_box_2d;
}
...
/*
// Commenting out - not declared in header
Box2d Polygon2d::MinAreaBoundingBox() const {  // NOLINT
  // CHECK_GE removed
  if (!is_convex_) {
...
Polygon2d Polygon2d::ExpandByDistance(const double distance) const {  // NOLINT
  if (!is_convex_) {
    Polygon2d convex_polygon;
    ComputeConvexHull(points_, &convex_polygon);
    ACHECK(convex_polygon.is_convex());
...
void RemoveDuplicates(std::vector<Vec2d>* points) {
  ...
  int count = 0;
  const double limit = kMathEpsilon * kMathEpsilon;
...
/*
// Commenting out - uses protobuf Polygon type
