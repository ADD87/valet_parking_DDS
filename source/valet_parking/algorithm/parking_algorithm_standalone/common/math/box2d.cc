...
Box2d::Box2d(const Vec2d& center, const double heading, const double length,
  ...
  if (length_ <= -kMathEpsilon) {
    AERROR << "Box2d length is invalid: " << length_;
  }
  if (width_ <= -kMathEpsilon) {
    AERROR << "Box2d width is invalid: " << width_;
  }
  InitCorners();
}
...
Box2d::Box2d(const LineSegment2d& axis, const double width)
    : center_(axis.center()),
      ...
      cos_heading_(cos(axis.heading())),
      sin_heading_(sin(axis.heading())) {
  if (length_ <= -kMathEpsilon) {
    AERROR << "Box2d length is invalid: " << length_;
  }
  if (width_ <= -kMathEpsilon) {
...
Box2d Box2d::CreateAABox(const Vec2d& one_corner,
  ...
  Box2d box_2d({(x1 + x2) / 2.0, (y1 + y2) / 2.0}, 0.0, x2 - x1, y2 - y1);
  return box_2d;
}
...
double Box2d::DistanceTo(const LineSegment2d& line_segment) const {  // NOLINT
  if (line_segment.length() <= kMathEpsilon) {
    ...
    switch (gx2 * 3 + gy2) {
      ...
      case -4:
        return CrossProd(x2 - x1, y2 - y1, box_x - x1, -box_y - y1) <= 0.0
                   ? PtSegDistance(box_x, -box_y, x1, y1, x2, y2,
                                   line_segment.length())
                   : (CrossProd(x2 - x1, y2 - y1, -box_x - x1, box_y - y1) <= 0.0
                          ? 0.0
                          : PtSegDistance(-box_x, box_y, x1, y1, x2, y2,
                                          line_segment.length()));
    }
  ...
  AERROR << "unimplemented state: " << gx1 << " " << gy1 << " " << gx2 << " "
         << gy2;
  return 0.0;
}
...
AABox2d Box2d::GetAABox() const {
  ...
  AABox2d aa_box_2d(center_, (dx1 + dx2) * 2.0, (dy1 + dy2) * 2.0);
  return aa_box_2d;
}
