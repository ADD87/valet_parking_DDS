...
double LineSegment2d::DistanceSquareTo(const Vec2d& point,
                                       ...
                                       std::string* const debug_info) const {
  if (nearest_pt == nullptr) {
    AERROR << "nearest_pt is nullptr";
    return -1.0;
  }
  if (length_ <= kMathEpsilon) {
    *nearest_pt = start_;
...
bool LineSegment2d::IsPointIn(const Vec2d& point) const {
  ...
  const double prod = CrossProd(point.x() - start_.x(), point.y() - start_.y(),
...
bool LineSegment2d::GetIntersect(const LineSegment2d& other_segment,
  ...
  const double cc1 = CrossProd(end_.x() - start_.x(), end_.y() - start_.y(),
                               other_segment.start().x() - start_.x(),
                               other_segment.start().y() - start_.y());
  const double cc2 = CrossProd(end_.x() - start_.x(), end_.y() - start_.y(),
                               other_segment.end().x() - start_.x(),
                               other_segment.end().y() - start_.y());
  if (cc1 * cc2 >= -kMathEpsilon) {
    return false;
  }
  const double cc3 = CrossProd(other_segment.end().x() - other_segment.start().x(),
                               other_segment.end().y() - other_segment.start().y(),
                               start_.x() - other_segment.start().x(),
                               start_.y() - other_segment.start().y());
  const double cc4 = CrossProd(other_segment.end().x() - other_segment.start().x(),
                               other_segment.end().y() - other_segment.start().y(),
                               end_.x() - other_segment.start().x(),
                               end_.y() - other_segment.start().y());
...
void MultiLineSegment::Init(const std::vector<Vec2d>& points) {
  ...
  if (num_points_ < 2) {
    AERROR << "MultiLineSegment requires at least 2 points, got " << num_points_;
    return;
  }
...
bool MultiLineSegment::GetProjection(const Vec2d& point,  // NOLINT
  ...
  if (num_points_ < 2) {
    AERROR << "MultiLineSegment requires at least 2 points for projection";
    return false;
  }
  int min_index = 0;
  if (num_points_ == 2) {
