...
Vec2d Vec2d::CreateUnitVec2d(const double angle) {
  Vec2d vec_2d(std::cos(angle), std::sin(angle));
  return vec_2d;
}
...
Vec2d Vec2d::rotate(const double angle) const {
  Vec2d vec_2d(x_ * cos(angle) - y_ * sin(angle),
...
Vec2d Vec2d::operator+(const Vec2d& other) const {
  Vec2d vec_2d(x_ + other.x(), y_ + other.y());
  return vec_2d;
}
...
Vec2d Vec2d::operator*(const double ratio) const {
  Vec2d vec_2d(x_ * ratio, y_ * ratio);
  return vec_2d;
}

Vec2d Vec2d::operator/(const double ratio) const {
  if (std::abs(ratio) <= kMathEpsilon) {
    AERROR << "Division by zero or very small number: " << ratio;
  }
  Vec2d vec_2d(x_ / ratio, y_ / ratio);
  return vec_2d;
}
