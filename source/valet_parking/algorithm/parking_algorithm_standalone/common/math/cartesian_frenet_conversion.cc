...
void CartesianFrenetConverter::cartesian_to_frenet(  // NOLINT
...
// TODO: This simplified version is not declared in header, commenting out
...
void CartesianFrenetConverter::frenet_to_cartesian(  // NOLINT
    const double rs, const double rx, const double ry, const double rtheta,
  ...
  if (std::abs(rs - s_condition[0]) >= 1.0e-6) {
    LOG(ERROR) << "The reference point s and s_condition[0] don't match: " << rs << " vs " << s_condition[0];
  }
...
// TODO: Following helper functions are not declared in header, commenting out
/*
...
double CartesianFrenetConverter::CalculateSecondOrderLateralDerivative(
  ...
  if (std::isinf(res)) {
    LOG(WARNING) << "result is inf when calculate second order lateral "
