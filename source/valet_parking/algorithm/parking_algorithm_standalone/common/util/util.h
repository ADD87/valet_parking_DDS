...
// #include "cyber/common/types.h" // 独立编译：已移除
// config_gflags.h is corrupted, commenting out for now
// #include "common/configs/config_gflags.h"
#include "common/math/vec2d.h"
#include "proto_convert/types_convert.h"
#include "proto_convert/pnc_point_convert.h"

// 独立编译改造：定义简化的cyber命名空间
namespace TL {
namespace cyber {
  constexpr int SUCC = 0;  // Success return code
}
}

/**
 * @namespace TL::common::util
 * @brief TL::common::util
 */
namespace TL {
namespace common {
namespace util {
...
bool ExcuteAllFunctions(T* obj, FunctionInfo<T> fun_list[]) {
  for (size_t i = 0; i < count; i++) {
    if ((obj->*(fun_list[i].function_))() != TL::cyber::SUCC) {
      LOG(ERROR) << fun_list[i].fun_name_ << " failed.";
      return false;
    }
  }
  return true;
