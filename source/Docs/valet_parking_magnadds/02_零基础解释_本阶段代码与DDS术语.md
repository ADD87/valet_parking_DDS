...
- 主输入 `/selected_slot`：告诉泊车“停哪个车位”。
- 辅助输入 `/localization/estimate`、`/chassis/state`、`/perception/obstacles`：告诉泊车“车在哪里、底盘什么状态、周围有什么障碍物”。

如果主输入坏了，泊车不知道目标车位，应该直接输出 `estop`。

如果辅助输入坏了，处理方式更像：

```text
这包定位不可信，先别用它。
这包底盘不可信，先清掉速度/档位补充信息。
这包障碍物不可信，先清空外部障碍物。
```

所以 NEXT-017 做的是“不要让坏的辅助信息污染规划状态”：

| 辅助输入模式 | 最简单解释 | 组件处理 |
|---|---|---|
| `all-valid` | 三包辅助信息都正常 | 使用外部车辆状态和障碍物 |
| `invalid-localization` | 定位说自己无效 | 清理外部车辆状态 |
| `nan-localization` | 定位坐标里有 NaN | 清理外部车辆状态 |
| `chassis-only` | 只有底盘，没有定位 | 不把车辆状态误认为有效 |
| `invalid-obstacles` | 障碍物数组说自己无效 | 清空外部障碍物 |
| `bad-obstacle-geometry` | 障碍物尺寸非法 | 拒绝并清空外部障碍物 |

一句话：
**主输入坏了要停车；辅助输入坏了先丢掉坏信息，不让它带歪后面的路径和速度计算。**
